...
$errMsg = "";
if(isset($_POST['atbtn'])){
	$atAmount = trim(mysqli_real_escape_string($conn,$_POST['amt']));
	$atItem = trim(mysqli_real_escape_string($conn,$_POST['atitem']));
	if($atAmount == "" || $atItem == ""){
		$errMsg = "All fields are required";
	}else{
		if(is_numeric($atAmount)){
			//Update database
			$q = "insert into asset (amount, assettype, userid) values('$atAmount', '$atItem','$userID')";
			mysqli_query($conn, $q);
...
<head>
  ...
  <title>Asset - Personal Finance Manager</title>
    ...
    <section class="section">
      <div class="row">
        <div class="col-lg-8">
          ...
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">Asset Form</h5>
			  <?php
				echo "<div class='pt-4 pb-2' style='color: #008800;text-align:center;'>$errMsg</div><hr>";
			  ?>
			  
              <form class="row g-3" method="post" action="asset.php">
                <div class="col-12">
                  <label for="inputEmail4" class="form-label">Asset Item</label>
                  <input type="text" class="form-control" id="inputEmail4" name="atitem" size="100">
                </div>
				<div class="col-12">
                  <label for="inputNanme4" class="form-label">Asset Value</label>
                  <input type="text" class="form-control" id="inputNanme4" name="amt">
                </div>
                <div class="text-center">
                  <button type="submit" class="btn btn-primary" name="atbtn">Submit</button>
                </div>
              </form>
